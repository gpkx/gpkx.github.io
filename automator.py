...
def call_ai_director(etf_list, time_label, report_type):
    ...
    if report_type == "weekly":
        prompt_context = f"今天【{DATE_STR}】是周末。请对本周触发涨跌幅阈值的排名前四只ETF进行纯粹的客观数据播报。"
    else:
        prompt_context = f"今天【{DATE_STR}{time_label}】有以下ETF触发涨跌幅阈值。请进行纯粹的客观数据播报。"

    if not etf_list:
        prompt = f"""
        你现在是一个没有任何感情的金融数据播报机器人。{prompt_context}
        今日没有任何ETF触发涨跌幅阈值。请生成通报文章。
        ...
        【输出要求】严格返回JSON，包含：
        - "video_intro": 短视频开场口播（20-30字，股民朋友，您好。我是李叔看盘+直接报出今天有几只涨跌幅阈值，以下是前四。）。🚨极端重要：必须全部使用纯中文生成！仅在提到ETF这三个字时，写成大写的ETF。
