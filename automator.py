...
def call_ai_director(etf_list, time_label, report_type):
    ...
    if not etf_list:
        ...
        - "gzh_title": 公众号标题。
        - "gzh_article": 公众号正文。
        - "cover_html": HTML5+CSS。720x1280。要求：现代金融风，浅色高级渐变背景。标题【{COVER_TITLE}】，副标题【{COVER_SUBTITLE}】。🚨必须在副标题下方，用醒目、美观的卡片或列表，把以上 4只ETF 的名称和涨跌幅数据排版渲染出来！所有字体设置为 'Alibaba PuHuiTi', 'Microsoft YaHei', sans-serif。居中对齐。
        """
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print("🌐 正在抓取 TV 原生图表 (处理水印注入与自适应缩放)...")
        ...
        for i, etf in enumerate(etf_list):
            ...
            # 🚀 注入图表中上位置 ETF名称+代码 (黑色, 50%透明, 40px)
            overlay_js = f"""
            let nameOverlay = document.createElement('div');
            nameOverlay.innerText = '{etf["name"]} ({etf["code"]})';
            nameOverlay.style.position = 'fixed';
            nameOverlay.style.top = '12%'; 
            nameOverlay.style.left = '50%';
            nameOverlay.style.transform = 'translateX(-50%)';
            nameOverlay.style.fontSize = '40px';
            nameOverlay.style.color = 'rgba(0, 0, 0, 0.5)';
            nameOverlay.style.fontWeight = 'bold';
            nameOverlay.style.fontFamily = '"Alibaba PuHuiTi", "Microsoft YaHei", sans-serif';
            nameOverlay.style.zIndex = '999999';
            nameOverlay.style.letterSpacing = '2px';
            document.body.appendChild(nameOverlay);
            """
            await page.evaluate(overlay_js)

            # 🚀 左对齐 (top left)，整体放大1.2倍涵盖了横向110%的要求，同时让所有文字元素更清晰
            zoom_js = """
            document.body.style.transform = 'scaleX(1.15) scaleY(1.2)';
